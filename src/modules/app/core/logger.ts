...
import { magenta, red, redBright, yellow } from "colorette"
import { createLogger, format, transports } from 'winston'
import { LoggingLevel, LoggingOptions } from '@/utils/interface';
    ...
    private HandleCreateLogger(level: LoggingLevel = "info") {
        return createLogger({
            format: combine(
                ...
                printf((info) => `[AIR-API] ${info.level.toUpperCase()} ${info.timestamp} - ${info.message}`)
            ),
